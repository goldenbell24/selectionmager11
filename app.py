...
st.subheader("노션의 상세 기록과 개인화된 가치관을 결합하여 최적의 경로를 분석합니다.")
...
def fetch_notion_full_data(api_key, database_id):
    ...
    # 1. 하이픈 자동 제거 및 정규식을 이용한 ID 추출
    # URL 형태인 경우 ID 부분만 추출하고, 하이픈(-)을 모두 제거합니다.
    clean_db_id = database_id.split('/')[-1].split('?')[0].replace("-", "").strip()
...
with st.sidebar:
    ...
    # 사용자가 하이픈이 있는 ID를 넣어도 괜찮음을 안내
    notion_db_id = st.text_input("Database ID", placeholder="하이픈 포함 가능")
    
    if st.button("🔌 노션 전체 데이터 불러오기", use_container_width=True):
        if notion_key and notion_db_id:
            with st.spinner("본문 내용까지 심층 수집 중..."):
                ...
                if "❌" not in st.session_state.notion_data:
                    st.success("노션 데이터 동기화 완료!")
                else:
                    st.error(st.session_state.notion_data)
        else:
...
if st.session_state.notion_data:
    with st.expander("📂 참조된 노션 심층 데이터", expanded=False):
        st.info("수집된 텍스트 데이터가 분석에 반영됩니다.")
        st.text_area("수집 내용 미리보기", value=st.session_state.notion_data, height=200)
...
if st.button("🚀 데이터 통합 정밀 분석 시작", use_container_width=True):
    ...
    if not openai_key:
        ...
        try:
            ...
            with st.spinner("노션 데이터와 현재의 가치를 대조 분석 중입니다..."):
                ...
                분석 지침:
                1. 노션 본문의 구체적인 내용을 바탕으로 각 선택지가 사용자의 과거 기록이나 수집 정보와 어떻게 일치하거나 충돌하는지 분석해줘.
                2. 설정된 가중치에 따라 가장 적합한 선택지 순위를 매겨줘.
                3. 결과는 Markdown 표와 명확한 리스트를 사용하여 작성해줘.
                """
